...
void init_matmul_testing(char *A_file, char *B_file, char *outfile)
{
        ...
        if(config.world_rank == 0){
			...
			printf("\n");
                ...
                //int starts[2] = {3,1};
                int starts[2] = {2,2};
                int littleSize[2] = {2,2};
                int bigSize[2] = {5,5};

                MPI_Datatype mysubarray;
                MPI_Type_create_subarray(2, bigSize, littleSize, starts, MPI_ORDER_C, MPI_DOUBLE, &mysubarray);
                ...
                MPI_Recv(littleKrabba, 2*2, MPI_DOUBLE, config.world_rank, config.world_rank, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

                printf("LittleArr:\n");
		for(i = 0; i < 2; i++){
                        for(j = 0; j < 2; j++){
                                printf("%f ", littleKrabba[i*2+j]);
                        }
                        printf("\n");
                }
